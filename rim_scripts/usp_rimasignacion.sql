...
BEGIN
	...
	SET @sql = @sql + N'bAnalisado BIT NULL)'
...
/*» Test ...*/
SELECT * FROM RimSimControlMigra2014
-- DROP TABLE RimSimControlMigra2014
DELETE FROM RimTablaDinamica WHERE sNombre = 'RimSimControlMigra2014'
EXEC dbo.usp_Rim_Procedimiento_NuevaTablaDinamica 'RimTest4'
/*-----------------------------------------------------------------------------------------------------------------------*/
...
/*» Test ...*/
EXEC dbo.usp_Rim_Procedimiento_ListarTablaDinamicaPorSufijoDeCampo 'Rim_Extraccion_2022', '_e'
/*-----------------------------------------------------------------------------------------------------------------------*/
...
BEGIN
	BEGIN TRY
		...
		DECLARE @count NVARCHAR(100) = N'SELECT COUNT(1) FROM ' + @nombreTabla
...
/*-----------------------------------------------------------------------------------------------------------------------*/
ALTER PROCEDURE dbo.usp_Rim_Procedimiento_AlterTablaDinamica
(
...
SELECT * FROM SidDependencia


/*» EXAMPLES: */
SELECT [value] FROM STRING_SPLIT('T e s t', ' ')


SELECT id FROM (VALUES (1), (2), (3)) AS tbl(id)

WITH cte AS (
	SELECT 
		so.nIdOperativo,
		DATEPART(yyyy, so.dFechaOperativo) nAñoOpe
	FROM SidOperativo so
)SELECT * FROM cte
PIVOT(
	COUNT(nIdOperativo) FOR nAñoOpe IN ([2019], [2020], [2021])
) pv

SELECT * FROM Rim_Pasaportes_2022

EXEC SP_EXECUTESQL N'SP_RENAME ''Rim_Pasaportes_2022.sSexo_a'', nSexo_a, ''COLUMN'' ALTER TABLE Rim_Pasaportes_2022 ALTER COLUMN nSexo_a int'
EXEC SP_EXECUTESQL N'SP_RENAME ''dbo.Rim_Pasaportes_2022.sSexo_a'', sSexo_a, ''COLUMN'' ALTER TABLE Rim_Pasaportes_2022 ALTER COLUMN sSexo_a INT'

EXEC SP_EXECUTESQL N'ALTER TABLE Rim_Pasaportes_2022 ALTER COLUMN sSexo_a int'

EXEC SP_EXECUTESQL N'ALTER TABLE Rim_Pasaportes_2022 ALTER COLUMN nSexo_a int'

EXEC SP_EXECUTESQL N'SP_RENAME ''dbo.Rim_Pasaportes_2022.sSexoo_a'', sSexo_a, ''COLUMN'' ALTER TABLE Rim_Pasaportes_2022 ALTER COLUMN sSexoo_a VARCHAR(MAX)'
...
SELECT TOP 1 * FROM Inpe_2022

ALTER TABLE Inpe_2022
ALTER COLUMN bTiene_Control_a VARCHAR(55) NULL

DELETE sup 
FROM SidUsuarioProcedimiento sup
JOIN SidUsuario su ON sup.uIdUsuario = su.uIdUsuario
JOIN SidProcedimiento sp ON sup.nIdProcedimiento = sp.nIdProcedimiento
WHERE 
	su.sLogin = 'rrguevarav'
	AND sp.nIdProcedimiento IN (55, 50, 49, 47)

UPDATE SidUsuario
SET sGrupo = 'ANALISIS'
WHERE sLogin = 'rrguevarav'
